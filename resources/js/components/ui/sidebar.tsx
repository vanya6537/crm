...
function SidebarInset({ className, children, ...props }: React.ComponentProps<"main"> & { children?: React.ReactNode }) {
  try {
    console.log('[SidebarInset] Called');
    console.log('[SidebarInset] children:', typeof children, Array.isArray(children) ? 'array' : 'not array');
    console.log('[SidebarInset] className:', className, typeof className);
    
    const baseClasses = "bg-background relative flex max-w-full min-h-svh flex-1 flex-col";
    const mergedClassName = className ? `${baseClasses} ${className}` : baseClasses;
    
    console.log('[SidebarInset] About to render main element');
